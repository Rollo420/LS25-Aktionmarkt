                    ...
                    @switch($type)
                        @case('buy')
                            <div class="w-10 h-10 bg-green-100 dark:bg-green-900 rounded-full flex items-center justify-center">
                                <svg class="w-5 h-5 text-green-900 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @if(in_array($type, ['buy', 'sell', 'dividend']))
                <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    @elseif(in_array($type, ['buy', 'sell']))
                        {{ number_format($transaction->quantity, 0, ',', '.') }} Aktien - {{ $transaction->stock?->name ?? '' }}
                    @endif
                </div>
            @endif
        </div>
    ...
    <div class="hidden sm:block overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Typ</th>
                    <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Details</th>
                    <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Betrag</th>
                    ...
                    <th class="px-3 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Datum</th>
                </tr>
            </thead>
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                    ...
                    <td class="px-3 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                        @if($type === 'dividend')
                            {{ number_format($transaction->quantity, 0, ',', '.') }}x {{ $transaction->stock?->name ?? 'Aktien' }} x {{ number_format($transaction->price_at_buy, 2, ',', '.') }} €
                        @elseif(in_array($type, ['buy', 'sell']))
